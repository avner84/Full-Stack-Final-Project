...
router.get("/confirmAccount", async (req, res) => {
    ...
    if (doseTokenMatch) {
        ...
        const email = tokenUserInformation.user.email;
...
const checkAuthHeader = (req, res, next) => {
    ...
    if (authHeader) {
        ...
        if (jsonWebToken.checkToken(token)) {
            const userFromToken = jsonWebToken.decryptToken(token);
...
router.get("/login_by_token", checkAuthHeader, (req, res) => {
    ...
    // const tokenInformation = jsonWebToken.decryptToken(req.token);
